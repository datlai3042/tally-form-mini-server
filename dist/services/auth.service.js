...
class AuthService {
    static async register(req, res, next) {
        ...
        if (foundEmail)
            throw new BadRequestError({ metadata: 'Email đã tồn tại' });
        ...
        if (!createUser)
            throw new ResponseError({ metadata: 'Không thể đăng kí user do lỗi' });
        const { private_key, public_key } = generatePaidKey();
        if (!public_key || !private_key)
            throw new ResponseError({ metadata: 'Server không thể tạo key sercet' });
        const payload = createPayload(createUser);
        const token = generatePaidToken(payload, { public_key, private_key });
        const code_verify_token = generateCodeVerifyToken();
        const { modelKeyQuery, modelKeyUpdate, modelKeyOption } = fillDataKeyModel(createUser, public_key, private_key, token.refresh_token, code_verify_token);
        const createKey = await keyManagerModel.findOneAndUpdate(modelKeyQuery, modelKeyUpdate, modelKeyOption);
        if (!createKey)
            throw new ResponseError({ metadata: 'Server không thể tạo model key' });
    ...
    static async login(req, res, next) {
        ...
        if (!foundUser)
            throw new NotFoundError({ metadata: 'Không tìm thấy user' });
        const checkPassword = compare(password, foundUser?.user_password);
        if (!checkPassword)
            throw new AuthFailedError({ metadata: 'Something wrongs...' });
